...
class StrategyEditor(tk.Frame):
    def __init__(self, root, binance: BinanceFuturesClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        self._base_params = [
            {"code_name": "strategy_type", "widget": tk.OptionMenu,
             "data_type": str, "values": ["Technical", "Breakout"], "width": 10, "header": "Strategy"},

            {"code_name": "contract", "widget": tk.OptionMenu,
             "data_type": str, "values": self._all_contracts, "width": 15, "header": "Contract"},

            {"code_name": "timeframe", "widget": tk.OptionMenu,
             "data_type": str, "values": self._all_timeframes, "width": 10, "header": "Timeframe"},
    ...
    def _delete_row(self, b_index: int):
        ...
        del self.additional_parameters[b_index]
        ...
        for param in self.extra_params[strategy_selected]:
            ...
            if param['widget'] == tk.Entry:
                ...
                if self.additional_parameters[b_index][code_name] is not None:
                    self._extra_input[code_name].insert(0, str(self.additional_parameters[b_index][code_name]))
            else:
                continue
    ...
    def _switch_strategy(self, b_index: int):
        ...
        for param in self.extra_params[strategy_selected]:
            code_name = param['code_name']
            if self.additional_parameters[b_index][code_name] is None:
